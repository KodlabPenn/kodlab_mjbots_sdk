...
class MjbotsRobotInterface {
 ...
 private:
  ...
  std::vector<std::reference_wrapper<const float>> positions_;  /// Vector of the motor positions
  std::vector<std::reference_wrapper<const float>> velocities_; /// Vector of the motor velocities
  std::vector<std::reference_wrapper<const float>> torque_cmd_; /// Vector of the torque command sent to motors
  std::vector<std::reference_wrapper<const ::mjbots::moteus::Mode>> modes_; /// Vector of the torque command sent to motors
  
  std::shared_ptr<bool> timeout_ = std::make_shared<bool>(false);                   /// True if communication has timed out
  std::vector<JointMoteus> joints_;
