 ...
 * @date 2022-07-12
 *
 * @copyright Copyright (c) 2021 The Trustees of the University of Pennsylvania.
...
class RobotBase {
 public:
  static const int KILL_ROBOT =
      -1;  // Kill mode/behavior index; used to signal robot E-stop
  std::vector<std::shared_ptr<JointBase>>
      joints;  /// the vector of shared_ptrs to joints
  std::vector<std::shared_ptr<LimbBase>>
      limbs;  /// vector of limbs for the robot, if choosing to not use limbs
              /// can be an empty vector always

  /*!
   * @brief constructs a robot_interface that contains the basic state of a
   * jointed robot with attitude
   * @param joint_vect a vector of shared pointers to jointbase defining the
   * motors in the robot
   ...
   * @param imu_data_ptr [Optional] Shared pointer to imu_data to use or nullptr
   * if robot should make its own
   */
  template <class JointDerived = JointBase>
  RobotBase(std::vector<std::shared_ptr<JointDerived>> joint_vect,
            float robot_max_torque, float soft_start_duration_ms,
            std::shared_ptr<::kodlab::IMUData<float>> imu_data_ptr = nullptr)
      : soft_start_(robot_max_torque, soft_start_duration_ms) {
    // Ensure at compile time that the template is JointBase or a child of
    // JointBase
  ...
  /*!
   * @brief accessor for joint positions, takes into account direction and
   * offset
  ...
  /*!
   * @brief Setter for the robot's IMU data pointer. Releases the previously
   * owned IMU data object
   *
   * @param imu_data_ptr a shared pointer to kodlab::IMUData
   */
  void SetIMUDataSharedPtr(
   ...
   */
  std::vector<std::shared_ptr<JointBase>> GetJoints(
      std::vector<int> joint_indices);

  /*!
   * @brief Get sub-vector of shared_ptr to joint objects via a set of indices
   * @param joint_indices set of desired joint indices std::initializer_list of
   * ints
   * @return a vector shared pointers to the desired joints
   */
  std::vector<std::shared_ptr<JointBase>> GetJoints(
      std::initializer_list<int> joint_indices);
  ...
  template <size_t N>
  std::vector<std::shared_ptr<JointBase>> GetJoints(
      std::array<int, N> joint_indices);

  /*!
   * @brief Get the vector of shared_ptrs to joints
   * @note Added getter to public member for interface consistency with
   * subvector getters
   * @param joint_indices set of desired joint indices std::initializer_list of
   * ints
   * @return a vector shared pointers to the desired joints
 ...
 protected:
  std::vector<std::reference_wrapper<const float>>
      positions_;  /// Vector of the motor positions (references to the members
                   /// of joints_)
  std::vector<std::reference_wrapper<const float>>
      velocities_;  /// Vector of the motor velocities (references to the
                    /// members of joints_)
  std::vector<std::reference_wrapper<const float>>
      torque_cmd_;  /// Vector of the torque command sent to motors (references
                    /// to the members of joints_)
  std::shared_ptr<::kodlab::IMUData<float>> imu_data_;  /// Robot IMU data
  real_time_tools::Timer
      run_timer_;         /// Run timer for robot, started at construction
